...
- Interface: a simple GUI that supports PDF selection, PDF open, and first-page preview.
- Distribution: a Windows executable artifact for end-user execution.

## Current Implementation State

- GUI implementation is provided with tkinter.
- Preview behavior is limited to the first PDF page.
- Conversion writes JPG files named with the pattern `<pdf-stem>-page-<page-number>.jpg`.
- Packaging is implemented as a PyInstaller one-folder Windows build under `dist/pdf-to-jpg-converter/`.
- Distribution can be performed by sending the built folder or its ZIP archive.
...
- The GUI shows a first-page preview for the selected PDF.
